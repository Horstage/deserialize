...
def _deserialize(class_reference, data, debug_name, throw_on_unhandled: bool, strict):
    ...
    if is_union(class_reference):
        ...
        for valid_type in valid_types:
            try:
                return _deserialize(
                    valid_type, data, debug_name, throw_on_unhandled, strict
                )
    ...
    if isinstance(data, dict):
        return _deserialize_dict(
            class_reference, data, debug_name, throw_on_unhandled, strict
        )
...
def _deserialize_list(
    class_reference, list_data, debug_name, throw_on_unhandled, strict
):
...
def _deserialize_tuple(
    class_reference, tuple_data, debug_name, throw_on_unhandled, strict
):

    if not isinstance(tuple_data, tuple):
        raise DeserializeException(
            f"Cannot deserialize '{type(tuple_data)}' as a list for {debug_name}"
        )

    if not is_tuple(class_reference):
        raise DeserializeException(
            f"Cannot deserialize a tuple to '{class_reference}' for {debug_name}"
        )
